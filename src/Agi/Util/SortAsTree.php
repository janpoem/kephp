...
// 实际数据库测试用例
$tree = Agi\Util\SortAsTree::factoryModel('Region', 'id', 'parent', 'position', 'name');
...
$tree = new \Agi\Util\SortAsTree($data, null, SORT_ASC);
$tree->sort(function ($tree, $index, $data, $depth) {
    echo str_repeat('->', $depth), $data['id'], '<br/>';
});
...
class SortAsTree
{
    ...
    // 每一节有什么节点
    private $levelNodes = array();

    // 每个节点属于第几级
    private $nodeLevels = array();
    ...
    public static function factoryModel(
        ...
        if (!is_subclass_of($model, '\\Adm\\Model'))
        ...
        if (!empty($select))
            $fields .= ' ,' . $select;
        $instance->setData($model::find(array('select' => $fields, 'array' => 1)));
        return $instance;
    }
    ...
    {
        $fields = array($this->primaryField, $this->parentField);
        if ($this->sortField !== false)
    ...
    {
        return $this->sortField === false ? false : array($this->sortField, $this->sortType, $this->sortMode);
    }
    ...
    public function prepare()
    {
        ...
        foreach ($this->data as $pk => $row) {
            if (!isset($row[$this->parentField]))
                throw new \Agi\Exception('invalid parent field!');
            if (!isset($row[$this->primaryField]))
                throw new \Agi\Exception('invalid primary field!');
            $parent = $row[$this->parentField];
            $sort = false;
            $node = $this->rootNode;
            if ($enableSort) {
                if (!isset($row[$this->sortField]))
                    throw new \Agi\Exception('invalid sort field!');
    ...
    public function loopNode($node, $callback, $depth = null, & $completed = array(), & $index = 0)
    {
