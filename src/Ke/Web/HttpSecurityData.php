...
trait HttpSecurityData
{
	...
	public function mkSecurityCode($prefix, array $fields = null)
	{
		if (session_status() !== PHP_SESSION_ACTIVE)
			throw new Exception('Use http security data should start session!');
		$code = $this->encrypt([microtime(true), KE_REQUEST_HOST, KE_APP_HASH, $prefix, $fields], KE_HTTP_SECURITY_SALT);
		if ($code === false)
	...
	public function prepareSecurity()
	{
		...
		if (empty($dec) || !is_array($dec) || count($dec) !== 5)
			return $this;
		list($this->securityMicrosecond, $rawHost, $rawHash, $this->securityPrefix, $this->securityFields) = $dec;
		if (KE_REQUEST_HOST === $rawHost && KE_APP_HASH === $rawHash) {
			$this->securityVerify = true;
			$pickData = depth_query($data, $this->securityPrefix, []);
