 ...
 * 简易的拼音处理转换器
 *
 * 这个拼音转换器，只实现了简单的声母韵母匹配和转换，并且结合了`CacheModelTrait`，可以视作`CacheModelTrait`的一个演示。
 *
 * 实际使用时：
 *
 * ```php
 ...
 * ```
 *
 * 存储拼音的格式，没有采用拼音，而是使用整型计算的。
 *
 * ```bash
 * 100 => a
 * 1  为音调，但因为字符集源没有音调所以也就没处理了，即所有都是1，都是第一声。
 * 00 表示的是单韵母发音，对应的a
 *
 * 10300 => bi
 * 03 表示的是韵母，对应的是 i
 ...
 * ```
 *
 * 这个存储表实际上可以支持9种音调，声母、韵母99位，当然可容纳是一回事，如何展现又是另一回事。整型存储和实际的拼音存储，容量差不多，
 * 而且这个类自带装载进缓存引擎。
 *
